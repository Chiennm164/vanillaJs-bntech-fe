...
$(document).ready(function() {
 ...
 $(window).load(function() {
    ...
    $(document).on('click' ,'.js_prd_popup',function(){
		...
		let ajax_url = 'homepage/ajax/home/prdPopup.html';
		$.ajax({
            url : ajax_url,
            type : "post",
            cache: 	false ,
	...
	$(document).on('click touch' ,'.js_buy' ,function(){
		...
		if(conditon == 'true'){
			...
			let ajax_url = 'cart/ajax/cart/addCart.html';
			$.ajax({
                url : ajax_url,
                type : "post",
                cache: 	false ,
	...
	$(document).on('click' ,'.ajax_add_prd_gift' ,function(){
		...
		$.ajax({
            ...
            data : {
            	...
            	toastr.success('Thêm sản phẩm vào giỏ hàng thành công','');
            	let ajax_url = 'cart/ajax/cart/refeshCart.html';
				$.ajax({
		            url : ajax_url,
		            type : "post",
		            cache: 	false ,
	...
	$(document).on('change','select[name=cityid], select[name=districtid]' , function(){
		let _this = $(this);
		let ajax_url = 'cart/ajax/cart/render_discount_ship.html';
		clearTimeout(time);
	...
	$(document).on('change','select[name=cityid], select[name=districtid]' , function(){
		let _this = $(this);
		let ajax_url = 'cart/ajax/cart/render_ship.html';
		clearTimeout(time);
		var time = setTimeout(function(){
    		$.ajax({
	            url : ajax_url,
...
function get_list_object(page){
	...
	let ajaxUrl = 'product/ajax/frontend/get_list_prd_cat.html';
	$.get(ajaxUrl, {
		page: param.page,
...
function getLocation(param, object){
	...
	let formURL = 'dashboard/ajax/dashboard/getLocation.html';
	$.post(formURL, {
...
function sub(a = 0 ,b = 0){
 ...
 $(window).load(function() {
	...
	$(document).on('click' ,'.js_del_coupon_payment' ,function(){
		let _this = $(this);
		let code_cp = _this.attr('data-coupon');
		let ajax_url = 'cart/ajax/cart/refeshPayment.html';
		$.ajax({
            url : ajax_url,
            type : "post",
            cache: 	false ,
